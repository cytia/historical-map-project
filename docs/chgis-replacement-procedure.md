...
验收通过后，同步更新 `docs/chgis-point-inventory.md` 的统计、区域状态、特殊映射和清单标记；更新 `docs/chgis-commercial-license.md` 的剩余 CHGIS 数量和授权邮件正文。商业构建只允许发布已完成区域，目标区域仍有 CHGIS `location` 主张时必须失败。
